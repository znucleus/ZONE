...
public class ExaminationNoticeExcel extends ReadExcelUtil<ExaminationNoticeDetail> {
    ...
    @Override
    public Optional<Map<String, String>> buildXlsxTitle(XSSFRow xssfRow) {
    ...
    @Override
    public Optional<Map<String, String>> buildXlsTitle(HSSFRow hssfRow) {
        Map<String, String> title = new HashMap<>();
    ...
    @Override
    public Optional<ExaminationNoticeDetail> buildXls(HSSFRow hssfRow) {
        Optional<ExaminationNoticeDetail> optional = Optional.empty();
        if (hssfRow.getLastCellNum() >= 13) {
            if (StringUtils.isNotBlank(getValue(hssfRow.getCell(0)))) {
                ExaminationNoticeDetail examinationNoticeDetail = new ExaminationNoticeDetail();
                ...
                optional = Optional.of(examinationNoticeDetail);
            }
        }
        return optional;
    }
    ...
    @Override
    public Optional<ExaminationNoticeDetail> buildXlsx(XSSFRow xssfRow) {
        Optional<ExaminationNoticeDetail> optional = Optional.empty();
        if (xssfRow.getLastCellNum() >= 13) {
