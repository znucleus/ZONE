//# sourceURL=timetable_course_edu_teacher_add.js
require(["jquery", "lodash", "tools", "handlebars", "sweetalert2", "nav.active", "messenger", "jquery.address", "check.all", "select2-zh-CN"],
    function ($, _, tools, Handlebars, Swal, navActive) {
        ...
        function initTeacherName(identification) {
            if (identification !== '') {
                $.get(ajax_url.teacher_name + '/' + identification, function (data) {
                    $(param_id.teacherName).html('<option label="请选择教师"></option>');
                    $(param_id.teacherName).select2({data: data.results});
                });
            } else {
                $(param_id.teacherName).html('<option label="请选择教师"></option>');
            }
        ...
        function initData(identification, teacherName) {
            if (identification !== '' && teacherName !== '') {
                $.get(ajax_url.data, {
        ...
        $(param_id.teacherName).change(function () {
            initParam();
            var v = $(this).val();
