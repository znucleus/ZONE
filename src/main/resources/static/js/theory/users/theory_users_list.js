//# sourceURL=theory_users_list.js
require(["jquery", "lodash", "tools", "handlebars", "nav.active", "sweetalert2", "responsive.bootstrap4",
    ...
    function ($, _, tools, Handlebars, navActive, Swal) {
        ...
        */
        var webStorageKey = {
            REAL_NAME: 'THEORY_USERS_LIST_REAL_NAME_SEARCH_' + page_param.paramTheoryReleaseId,
            STUDENT_NUMBER: 'THEORY_USERS_LIST_STUDENT_NUMBER_SEARCH_' + page_param.paramTheoryReleaseId,
            USERNAME: 'THEORY_USERS_LIST_USERNAME_SEARCH_' + page_param.paramTheoryReleaseId
        };
        ...
        function getAjaxUrl() {
            return {
                ...
                remark: web_path + '/web/theory/users/remark',
                reset: web_path + '/web/theory/users/reset',
        ...
        var myTable = tableElement.DataTable({
            ...
            stateSaveCallback: function (settings, data) {
                localStorage.setItem('TRAINING_USERS_' + settings.sInstance, JSON.stringify(data))
            },
            stateLoadCallback: function (settings) {
                return JSON.parse(localStorage.getItem('TRAINING_USERS_' + settings.sInstance))
            },
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (a, b, c, d) {
                        return '<input type="checkbox" value="' + c.theoryUsersId + '" name="check"/>';
                    }
                },
                {
                    targets: 10,
            ...
            initComplete: function () {
                tableElement.delegate('.del', "click", function () {
                    theory_del($(this).attr('data-id'), $(this).attr('data-student'));
                });
        ...
        var global_button = '';
        if (Number(page_param.canOperator) === 1) {
            global_button += '<button type="button" id="theory_add" class="btn btn-outline-primary btn-sm"><i class="fa fa-plus"></i>添加</button>' +
                '  <button type="button" id="theory_dels" class="btn btn-outline-danger btn-sm"><i class="fa fa-trash-o"></i>批量删除</button>' +
                '  <button type="button" id="theory_reset" class="btn btn-outline-warning btn-sm"><i class="fa fa-reply-all"></i>重置</button>';
        }
        ...
        */
        ...
        /*
         批量删除
         */
        $('#theory_dels').click(function () {
            var theoryUsersIds = [];
            var ids = $('input[name="check"]:checked');
            for (var i = 0; i < ids.length; i++) {
                theoryUsersIds.push($(ids[i]).val());
            }

            if (theoryUsersIds.length > 0) {
                Swal.fire({
                    title: "确定删除选中的学生吗？",
                    text: "学生删除！",
                    type: "warning",
        ...
        */
        ...
        /*
         删除
         */
        function theory_del(theoryUsersId, realName) {
            Swal.fire({
        ...
        function del(theoryUsersId) {
            sendDelAjax(theoryUsersId);
        }

        function dels(theoryUsersIds) {
            sendDelAjax(theoryUsersIds.join(","));
        }
         ...
         * @param theoryUsersId
         */
        function sendDelAjax(theoryUsersId) {
            $.ajax({
                type: 'POST',
                url: getAjaxUrl().del,
                data: {theoryUsersIds: theoryUsersId, theoryReleaseId: page_param.paramTheoryReleaseId},
        ...
        function sendResetAjax() {
            $.ajax({
                ...
                data: {theoryReleaseId: page_param.paramTheoryReleaseId},
                success: function (data) {
                    Messenger().post({
                        message: data.msg,
                        type: data.state ? 'success' : 'error',
       ...
       备注
       */
        function remark(theoryUsersId, remark) {
            $('#editRemark').text(remark);
            $('#editTheoryUsersId').val(theoryUsersId);
